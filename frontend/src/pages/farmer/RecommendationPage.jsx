...
import { Loader, Info, Leaf, CloudSun, Crosshair } from 'lucide-react';
...
export default function RecommendationPage() {
  ...
  const formattedAlternative = (() => {
    if (!r?.alternative) return null;
    if (typeof r.alternative === 'string') return r.alternative;
    if (r.alternative.startLabel && r.alternative.endLabel) {
      return `${r.alternative.startLabel} – ${r.alternative.endLabel}`;
    }
    if (r.alternative.start && r.alternative.end) {
      return `${r.alternative.start} – ${r.alternative.end}`;
    }
    return null;
  })();
      ...
      <div ref={containerRef} className="text-[#09090b] page-enter">
        <div className="mb-10">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-medium tracking-tight text-[#09090b] leading-[1.08]">
            Kalkulator Kalender Tanam
          </h1>
          <p className="text-base sm:text-lg text-[#71717a] mt-3 max-w-2xl leading-relaxed">
            Masukkan parameter lahan dan varietas benih untuk memproyeksikan jadwal tanam optimal dan mitigasi risiko cuaca BMKG.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          {r && (
            <div className="space-y-4 reveal-up">
              <div className="rounded-2xl bg-white border border-[#e4e4e7] p-5 sm:p-6 shadow-sm relative overflow-hidden" style={{ borderLeftWidth: 4, borderLeftColor: r.riskColor || '#10b981' }}>
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <StatusBadge status={r.status || 'LAYAK TANAM'} />
                ...
                {r.window && (
                  <div className="p-4 bg-emerald-50/60 rounded-xl border border-emerald-200 mb-4">
                    ...
                    {r.window.end && <div className="text-sm text-[#71717a]">sampai {r.window.end}</div>}
                  </div>
                )}

                <ConfidenceBar value={r.confidence || 85} className="mb-4" />
                ...
                <div className="mb-4">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex-1 h-2 bg-[#f4f4f5] rounded-full overflow-hidden border border-[#e4e4e7]">
                      <div className="h-full rounded-full transition-all duration-500" style={{ width: `${r.riskScore || 30}%`, backgroundColor: r.riskColor || '#10b981' }} />
                    </div>
                    <RiskBadge label={r.risk || 'Aman'} badge={r.riskBadge || 'Aman'} />
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div className="p-3 bg-[#fafafa] rounded-xl border border-[#e4e4e7]">
                    ...
                    <p className="text-xs sm:text-sm text-[#09090b] leading-relaxed">{r.reason || 'Kondisi cuaca BMKG relatif stabil untuk masa penanaman.'}</p>
                  </div>
                  ...
                  <div className="p-3.5 bg-emerald-50/50 rounded-xl border border-emerald-100">
                    ...
                    <p className="text-xs sm:text-sm font-medium text-emerald-950">{r.action || 'Lanjutkan pemantauan kelembapan tanah dan saluran irigasi.'}</p>
                  </div>

                  {formattedAlternative && (
                    <div className="flex items-center gap-2 text-xs text-[#71717a]">
                      <span className="text-amber-600 font-medium">Alternatif:</span>
                      <strong className="text-[#09090b] font-medium">{formattedAlternative}</strong>
                    </div>
                  )}
              ...
              {r.details && (
                <div className="rounded-2xl bg-white border border-[#e4e4e7] p-5 shadow-sm">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      { label: 'Rata-rata peluang hujan', value: `${r.details.avgRainProbability ?? 0}%` },
                      { label: 'Suhu rata-rata', value: `${r.details.avgTemperature ?? 0}°C` },
                      { label: 'Kelembapan rata-rata', value: `${r.details.avgHumidity ?? 0}%` },
                      { label: 'Hari hujan berturut-turut', value: `${r.details.maxConsecutiveWetDays ?? 0} hari` },
                      { label: 'Periode kering terpanjang', value: `${r.details.maxDrySpell ?? 0} hari` },
                      { label: 'Hari hujan ekstrem', value: `${r.details.extremeRainDays ?? 0} hari` },
                    ].map((f, i) => (
